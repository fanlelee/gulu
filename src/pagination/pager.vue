<template>
    <div class="gulu-pager">
        ...
        <template v-for="page in pages">
            ...
            <template v-else>
                <span class="gulu-pager-item others" @click="onClickPage(page)">{{page}}</span>
            </template>
        </template>
        <span class="gulu-pager-nav next"
...
<style scoped lang="scss">
    ...
    .gulu-pager {
        ...
        &-item {
            ...
            margin: 0 4px;font-size: $font-size;border-radius: $border-radius;user-select: none;
            &.active {color: #2295FE;border-color: #2295FE;cursor: default;}
            &.others {
                &:hover {color: #2295FE;border-color: #2295FE;}
            }
        }
        .dots-icon {width: $width;font-size: $font-size;}
        &-nav {
            ...
            svg {fill: lighten(#000, 30%);}
            &.pre {margin-right: 4px;}
            &.next {margin-left: 4px;}
            &:hover {border-color: #2295FE;
                svg {fill: #2295FE;}
            }
            &.disabled {cursor: not-allowed;
                svg {fill: darken($grey, 20%);}
                &:hover {border-color: darken($grey, 20%);
                    svg {fill: darken($grey, 20%);}
                }
            }
        }
